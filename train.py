...
from datasets.neontree import NeonTree, Neon_eval
...
from utils.post_process import ctdet_decode, circledet_decode
...
parser.add_argument('--dataset', type=str, default='coco', choices=['coco', 'pascal','neontree'])
...
def main():
  ...
  if cfg.dataset == 'coco':
    ...
    Dataset_eval = Neon_eval
  else:
    Dataset_eval = PascalVOC_eval
  ...
  if torch.cuda.is_available():
    cfg.device = torch.device('cuda:0')  # Ensure the device is set to GPU
  else:
    cfg.device = torch.device('cpu')  # If CUDA is not available, fallback to CPU
  ...
  model = model.to(cfg.device) #new
  
# If using DataParallel, wrap the model with it
  ...
  if cfg.dist:
    ...
    model = nn.DataParallel(model).to(cfg.device)
    print(f'Model is on device: {next(model.parameters()).device}')  # This should print cuda:0 if on GPU
  ...
  def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
      ...
      hmap, regs, radius = zip(*outputs)
      regs = [_tranpose_and_gather_feature(r, batch['inds']) for r in regs]
      # w_h_ = [_tranpose_and_gather_feature(r, batch['inds']) for r in w_h_]
      # print("pre radius    ", radius)
      radius = [_tranpose_and_gather_feature(r, batch['inds']) for r in radius]

      hmap_loss = _neg_loss(hmap, batch['hmap'])
      reg_loss = _reg_loss(regs, batch['regs'], batch['ind_masks'])
      # w_h_loss = _reg_loss(w_h_, batch['w_h_'], batch['ind_masks'])
      # print("1   ", batch['radius'].shape)
      # print("2   ", radius[0].shape)
      # print("1   ", batch['radius'])
      # print("len    ", len(radius))
      # print("2   ", radius[0])
      radius_loss = _reg_loss(radius, batch['radius'], batch['ind_masks'])
      # loss = hmap_loss + 1 * reg_loss + 0.1 * w_h_loss
      loss = hmap_loss + 1 * reg_loss + 0.1 * radius_loss
      ...
      if batch_idx % cfg.log_interval == 0:
        ...
        print('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
              ' hmap_loss= %.5f reg_loss= %.5f radius_loss= %.5f' %
              # (hmap_loss.item(), reg_loss.item(), w_h_loss.item()) +
              (hmap_loss.item(), reg_loss.item(), radius_loss.item()) +
        ...
        summary_writer.add_scalar('radius_loss', radius_loss.item(), step)
    return
    ...
    with torch.no_grad():
      for inputs in val_loader:
        ...
        for scale in inputs:
          ...
          # print("output    ", output)
          dets = circledet_decode(*output, K=cfg.test_topk)
          print(f"Before NMS, number of detections: {dets.shape[0]}")
        ...
        # Debugging: Print how many detections are kept after applying the threshold
        print(f"Number of detections after score thresholding: {len(scores)}")
