...
class PoseResNet(nn.Module):
  def __init__(self, block, layers, head_conv, num_classes):
    ...
    if head_conv > 0:
      ...
      #                           nn.ReLU(inplace=True),
      #                           nn.Conv2d(head_conv, 2, kernel_size=1, bias=True))
      self.r = nn.Sequential(nn.Conv2d(64, head_conv, kernel_size=3, padding=1, bias=True),
                                nn.ReLU(inplace=True),
  ...
  def forward(self, x):
    ...
    out = [[self.hmap(x), self.regs(x), self.r(x)]]
    # out = [[self.hmap(x), self.regs(x), self.act(self.r(x))]]
    return out

  def init_weights(self, num_layers):
    url = model_urls['resnet{}'.format(num_layers)]
    print("num layers    ", num_layers)
    with open('resnet50_weights.pkl','rb') as f:
      resnet_weights = pickle.load(f)
    # resent = resent50()
    # resent.load_state_dict(resent_weights, strict=false)
    # kkk += 4
    # pretrained_state_dict = model_zoo.load_url(url)
    # print('=> loading pretrained model {}'.format(url))
    # self.load_state_dict(pretrained_state_dict, strict=False)
    self.load_state_dict(resnet_weights, strict=False)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # ✅ <-- This line ensures all weights move to GPU
    self.to(device)  # ✅ <-- This line ensures all weights move to GPU
...
def get_pose_net(num_layers, head_conv=64, num_classes=80):
  ...
  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # optional double check
  model = model.to(device)  # optional double check
  return model
